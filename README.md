...
- в эту выгрузку **не входят диалоги Direct-кампаний** (~5) — с ними начатых переписок **~32**;
...
- его **кликают выше нормы и по недвижимости, и по финансам** (CTR рилов ~3%, каруселей ~5,3% против [1,68% в недвижимости и 0,98% в финансах](research-appendix.md#11-бенчмарки-кликабельности-и-охвата-контента) — нормы США, 2025),
...
Ощущение «одно и то же по кругу» возникает из-за **тестирования заходов внутри гипотез**. Со стороны ролики на одну JTBD похожи — потому что JTBD одна и та же. Но между заходами меняется главное — **смысловая конструкция**.

Цикл такой: гипотеза → несколько заходов на неё (тема / боль / подача) → каждый запускаем дешёвым тестом (обычно ~$10, 5–6 дней) → снимаем **посекундную карту смыслов** → сильные заходы оставляем в работе, слабые отсекаем. Это не «повтор» — это перебор заходов с чтением данных между ними. [Подробно в отчёте](content-effectiveness.md#3-тестирование-заходов--почему-кажется-что-снимаем-одно-и-то-же).

Дальше — следующий этап: **докрутка внутри сценария** по накопленным картам смыслов, параллельно с лидгеном. Для тебя это станет легче: **контента меньше, темы разнообразнее**.
...
3. По технологии и рекомендации FB нужно набрать ~50 событий. **Почему отключили раньше?** Наш опыт по поведению кампаний: мы по совокупности метрик видим, как кампания развивается в первые дни, и когда вероятность, что она «разгонится», низкая — отключаем, чтобы не сливать бюджет. Это ложится на исследования: в доверительных нишах прямой лидген «в лоб» по холодной аудитории работает хуже прогрева — [подтверждено на 996 кейсах за 30 лет](research-appendix.md#1-прогрев-доверия-до-лидгена).
...
И отдельно про баг в тик-токе: справедливо — **его первыми заметили вы**. Спасибо, что смотрите так внимательно, это реально помогает. А теперь честно о том, как устроен контроль на этом этапе. Сейчас мы **просто постим** в эти каналы. Контроль есть, но он очень поверхностный — идёт постинг или нет и нет ли провалов по органике. Провал — это когда все ролики перестали набирать среднюю статистику. Такое бывает: например, тик-ток скидывает просмотры просто в ноль, и у ютуба тоже бывают загоны. Вот это мы контролируем. Но контент на канале не отсматривается. Причина простая — в этом нет смысла на этом этапе.
...
Я думаю, тут ещё добавилось то, что ты взял на свою сторону больше, чем мы просили: отснял целую кучу коротких роликов (они помогли, спасибо большое), но они до сих пор не все пригодились и висят в запасе — а ты их снимал. Ещё добавить монтаж и обложки, которые вы делали, — вот и выгорание. Поинт простой: **нам надо лучше координироваться**.

И обязательно посмотри [подробный отчёт по контенту](content-effectiveness.md) — чтобы понимать, как мы действуем.
...
Почему не крутим контент повторно на тех, кого уже собрали? Потому что **на этапе прогрева это не работает**. Ретаргет по той же аудитории — это просто платить Facebook в ~4 раза дороже без смысла. Ретаргетинг имеет смысл только для **лидогенерящих кампаний** — и вот там мы его обязательно включим, на этапе закрытия. И важно: вся эта аудитория **никуда не пропадает** — каждый, кто смотрел ролики, заходил в профиль или писал, копится в пикселе и вовлечённых аудиториях Меты. Это и есть **база, по которой запустим лидген**: мы её не бросаем — мы её складируем для работы лидген-кампаний. И мы с ней работаем нашими текущими кампаниями.
...
А в иммигрантских сообществах доверие вообще строится годами: владение жильём у иммигрантов растёт [с 16% до 61% — за 20+ лет в стране](research-appendix.md#8-доверие-и-свой-эксперт-в-иммигрантских-сообществах). Масштаб задан: 3–4 месяца узнавания — это только-только начало.
...
И вывод «надо делать лидген» — верный. Вопрос только в том, **как его делать, когда и какими методами**. Если в лоб — это не работает: мы уже видим это на цифрах, так как пробовали, и [анализ 996 маркетинговых кейсов](research-appendix.md#1-прогрев-доверия-до-лидгена) говорит о том же.
...
Сайт и 5 лендингов — то, что мы построили под лидген. Хронология:
...
| Готовы 5 лендингов: pre-approval, аренда, взнос 20%, texas, equity | 17.06 |
...
Ты просишь ввести **согласования до запуска** — по аудиториям, креативам, бюджетам. Как думаешь: если мы введём согласования до запуска — сможем работать эффективно и без блокеров при твоей загрузке?
...
А разгрузить тебя от съёмок можно **по-другому** (и это не HeyGen): делать **меньше контента и дольше его крутить** — это уже разгружает; часть контента **монтировать без твоего участия**. Это варианты, которые нужно обсудить и согласовать — если устроит, внедряем. А тупой [«контент-завод» не работал год назад и не работает сейчас](research-appendix.md#7-контент-фабрики-объём-не-работает).

---

## 6. Клиенты уходят от маркетинговых агентств из-за отчётов

Есть интересное исследование: [73% ушедших клиентов называют причиной коммуникацию, а не результат](research-appendix.md#5-почему-портятся-отношения-с-клиентом). Мы это уже давно обнаружили. То есть причина — не рукожопость маркетологов (скорее всего, не только лишь она). Механика простая: у клиента нет понимания, что происходит → появляется ощущение потери контроля → тревожность. А самый простой способ решить проблему потери контроля — уйти. И самый простой способ для маркетолога эту проблему снять — наладить отчёты.

На сборку отчёта, который ты видишь, ушло два полных дня работы. Я не оч люблю отчёты и с этим риском — потеря клиента от тревожности непонимания процесса — мирюсь. У нас 7 клиентов в работе, маленькая команда: если я введу такие отчёты — не смогу давать нормальный результат. Я оч люблю свою работу, она меня заряжает, поэтому я могу сутками ковыряться с сайтом или анализом контента. А отчёты меня фрустрируют. Поэтому отчёты я делаю только по запросу, а ресурс предпочитаю тратить на деливеры для клиента. Предлагаю компромисс по отчётам: я сделаю **автоматизированную форму, еженедельную или ежемесячную**, которую мы с тобой согласуем. А ручной отчёт типа этого — **по запросу**, когда в следующий раз почувствуешь, что потерял нить. Мы, собственно, так работаем со всеми нашими клиентами.

Если посмотреть объективно на наши результаты, сроки, объём выполненных работ и цену — мы **охеренное решение для твоего бизнеса**. Я сомневаюсь, что ты найдёшь что-то близкое по этой цене. Я не думаю, что найденный тобой специалист сможет дать даже близко наш уровень — не то что проработки, а просто вовлечённости. Решение, конечно, за тобой.

---

## Приложения

- [Переход на Loan Factory — полный список правок сайта (июль 2026)](loan-factory-rebrand.md)
- [Ссылки на сайт для соцсетей с UTM-метками — куда поставить](utm-links-social.md)
- [Эффективность контента — по данным рекламного кабинета Facebook](content-effectiveness.md)
- [Что мы построили: сайт mortgageservicepro.com](site.md)
- [Голоса аудитории — дословные цитаты с форумов иммигрантов](audience-voices.md)
